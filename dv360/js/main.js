...
async function handleSendMessage(message) {
    ...
    // Guardar productos en el estado para el email
...
// NUEVA FUNCIÓN: Enviar wishlist por email
function sendWishlistByEmail(products) {
    ...
    if (typeof Enabler !== 'undefined' && Enabler.isInitialized()) {
        Enabler.counter('Email Sent');
    }
    
    // Generar contenido del email
    const subject = encodeURIComponent('Mi Wishlist de San Valentín ❤️');
    
    let emailBody = 'Hola mi amor ❤️\n\n';
    ...
    if (bubble) {
        bubble.innerHTML = `
            <p class="greeting-text">
                <strong>💌 ¡Wishlist lista para enviar!</strong>
            </p>
            <p class="sub-text">
                Se abrirá tu cliente de email con la wishlist completa
            </p>
        `;
...
function displayProducts(products) {
    ...
    // Agregar botón de enviar wishlist por email
    if (products.length > 0) {
        const emailBtn = document.createElement('button');
        emailBtn.className = 'email-wishlist-btn';
        emailBtn.innerHTML = '💌 Enviar Wishlist por Email';
        emailBtn.onclick = () => sendWishlistByEmail(products);
